...
function encodeQueryParams(sourceUrl, campaignId, userId) {
  const data = JSON.stringify({ sourceUrl, campaignId, userId });
  const encodedParams = btoa(data); // Encode to Base64
...
router.post('/campaigns/joinCampaign', async (req, res) => {
  try {
    ...
    const encodedParams = encodeQueryParams(campaignData.sourceLink, campaignId, userId);
    // const sublink = `https://share2grow-server.onrender.com/share2grow/track?data=${encodedParams}`;
    const sublink = `http://localhost:4000/share2grow/track?data=${encodedParams}`;
...
function decodeQueryParams(encodedData) {
  const decodedString = Buffer.from(encodedData, 'base64').toString('utf-8');
  return JSON.parse(decodedString);
}

// Helper function to update the user's wallet
async function addToWallet(userDocRef, campaignData, campainId, businessId) {
 
  const walletField = campaignData.campaignType?.type === "cash" ? "cash" : "dsicount";
  const walletEntry = {
    businessId,
    campainId,
    value: campaignData.campaignType?.value || 0,
  };

  await updateDoc(userDocRef, {
    [`wallet.${walletField}`]: arrayUnion(walletEntry),
  });
}

// Helper function to remove a campaign from joinedCampaigns
async function removeFromJoinedCampaigns(userDocRef, campaignId) {
  ...
  const updatedCampaigns = (userData.joinedCampaigns || []).filter(
    (campaign) => campaign.campaignId !== campaignId
  );

  await updateDoc(userDocRef, { joinedCampaigns: updatedCampaigns });
}
...
router.get('/track', async (req, res) => {
  ...
  try {
    ...
    let usedIpArray = campaignData.used_ip || [];

    // Check if the IP and user-agent exist
    ...
    if (ipEntryIndex !== -1) {
      ...
      if (!userAgents.includes(userAgent)) {
        ...
        await updateDoc(campaignDocRef, { used_ip: usedIpArray });
    ...
    // Check if collectedShares equals allShares
  
    const userData = userSnapshot.data();
    const joinedCampaign = (userData.joinedCampaigns || []).find(c => c.campaignId === campaignId);
    console.log(joinedCampaign)

    if (joinedCampaign && joinedCampaign.collectedShares + 1 === joinedCampaign.allShares) {
      // Add to wallet
      await addToWallet(userDocRef, campaignData, campaignId, campaignData.businessId);

      // Remove campaign from joinedCampaigns
      await removeFromJoinedCampaigns(userDocRef, campaignId);
    }
...
router.post('/Customer-dashboard-data', async (req, res) => {
  const { currentUserUid } = req.body;

  if (!currentUserUid) {
      return res.status(400).send({ error: "Missing 'currentUserUid' in request body" });
  }

  try {
      // Reference the user's document in the Firestore
